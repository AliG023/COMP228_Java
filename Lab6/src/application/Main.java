...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		...
		employmentForm.add(firstNameField, 0, 2);
		employmentForm.add(new Label("Last Name *:"), 1, 1);
		TextField lastNameField = new TextField();
		employmentForm.add(lastNameField, 1, 2);
		...
		employmentForm.add(positionField, 0, 8);
		employmentForm.add(new Label("Salary Requirements:"), 0, 9);
		TextField salaryField = new TextField();
		employmentForm.add(salaryField, 0, 10);
		employmentForm.add(new Label("When Can You Start [YYYY-MM-DD]:"), 1, 9);
		TextField startDateField = new TextField();
		employmentForm.add(startDateField, 1, 10);
		employmentForm.add(new Label("Phone *:"), 0, 11);
		TextField phoneField = new TextField();
		employmentForm.add(phoneField, 0, 12);
		employmentForm.add(new Label("Fax:"), 1, 11);
		TextField faxField = new TextField();
		employmentForm.add(faxField, 1, 12);
		...
		employmentForm.add(companyField, 0, 16);
		...
		submitHandlerClass submitHandler = new submitHandlerClass(
				...
				portfolioField, positionField, salaryField,
				startDateField, phoneField, faxField,
				companyField);
		submitButton.setOnAction(submitHandler);

		// Read Button ---------------------------------------
		Button readButton = new Button("Read Applicants");
		readHandlerClass readHandler = new readHandlerClass(commentArea);
		readButton.setOnAction(readHandler);

		buttonBox.getChildren().addAll(submitButton, readButton);
